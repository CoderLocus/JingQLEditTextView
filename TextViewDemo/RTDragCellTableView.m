...
- (void)didEndDraging{
    ...
    NSLog(@"%ld --- %ld", _firstIndexPath.row, _relocatedIndexPath.row);
    if ([self.delegate respondsToSelector:@selector(didFinishDragingTableView:isMovedIndexPath:toIndexPath:)]) {
        [self.delegate didFinishDragingTableView:self isMovedIndexPath:_firstIndexPath toIndexPath:_relocatedIndexPath];
    }
}
